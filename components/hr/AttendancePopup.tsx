...
interface AttendancePopupProps {
  currentUser: User;
  onCheckIn: (wasLate?: boolean) => void;
}
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {!checkedIn ? (
            ...
            >
              ...
              {isLate && (
                <p className="text-[10px] text-red-600 dark:text-red-400 font-semibold mt-2">
                  Your manager has been notified about this late arrival.
                </p>
              )}
            </div>
          )}

          {/* ═══ Daily Target Briefing (after check-in) ═══ */}
          {checkedIn && currentUser.target && (
            <div className="panel p-4 space-y-3 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs text-text-secondary">Monthly Collection Target</span>
                    <span className="text-xs font-bold" style={{ color: ORANGE }}>AED {currentUser.target?.toLocaleString()}</span>
                  </div>
                  <div className="w-full h-2.5 rounded-full bg-[var(--color-bg-tertiary)] overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: '0%', background: ORANGE, transition: 'width 1s ease-out' }} />
                  </div>
                  <p className="text-[10px] text-text-tertiary mt-1">Start making calls to hit your target!</p>
                </div>
              </div>
            </div>
          )}
