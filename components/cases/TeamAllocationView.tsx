...
import { formatCurrency, formatDate, exportToCsv, convertToAED } from '../../utils';
...
const TeamAllocationView: React.FC<TeamAllocationViewProps> = ({ cases, coordinators, onSelectCase, onReassign }) => {
  ...
  const [bankOfficerMap, setBankOfficerMap] = useState<Record<string, string>>({});
  ...
  const bankStats = useMemo(() => {
    const stats: Record<string, { bank: string; caseCount: number; totalOutstanding: number; caseIds: string[] }> = {};
    assignableCases.forEach(c => {
      const bank = c.loan?.bank || 'Unknown';
      if (!stats[bank]) {
        stats[bank] = { bank, caseCount: 0, totalOutstanding: 0, caseIds: [] };
      }
      stats[bank].caseCount++;
      stats[bank].totalOutstanding += convertToAED(c.loan?.currentBalance || 0, c.loan?.currency || 'AED');
      stats[bank].caseIds.push(c.id);
    });
    return Object.values(stats).sort((a, b) => b.caseCount - a.caseCount);
  }, [assignableCases]);

  const handleBankOfficerChange = (bank: string, officerId: string) => {
    setBankOfficerMap(prev => ({ ...prev, [bank]: officerId }));
  };

  const handleBankAssignAll = (bank: string) => {
    const officerId = bankOfficerMap[bank];
    if (!officerId) {
      alert('Please select a coordinator first.');
      return;
    }
    const stat = bankStats.find(s => s.bank === bank);
    if (!stat || stat.caseIds.length === 0) return;
    onReassign(stat.caseIds, officerId);
    setBankOfficerMap(prev => { const next = { ...prev }; delete next[bank]; return next; });
  };
  ...
  return (
    <div className="p-6 bg-background min-h-full">
      ...
      <div className="mb-6 max-w-sm h-fit">
        <KpiCard
      ...
      {bankStats.length > 0 && (
        <Card className="mb-6 p-6">
          <h2 className="text-xl font-bold text-text-primary mb-4">Bank-wise Allocation</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {bankStats.map(stat => (
              <div key={stat.bank} className="border border-border rounded-lg p-4 bg-surface-muted">
                <h3 className="text-sm font-bold text-text-primary truncate" title={stat.bank}>{stat.bank}</h3>
                <div className="mt-2 space-y-1">
                  <p className="text-xs text-text-secondary">Unassigned Cases: <span className="font-semibold text-warning">{stat.caseCount}</span></p>
                  <p className="text-xs text-text-secondary">Total Outstanding: <span className="font-semibold text-red-600">{formatCurrency(stat.totalOutstanding, 'AED')}</span></p>
                </div>
                <div className="mt-3">
                  <label className="block text-xs font-medium text-text-secondary mb-1">Auto-assign to:</label>
                  <select
                    value={bankOfficerMap[stat.bank] || ''}
                    onChange={e => handleBankOfficerChange(stat.bank, e.target.value)}
                    className="block w-full pl-2 pr-8 py-1.5 text-xs text-text-primary bg-surface border-border focus:outline-none focus:ring-accent focus:border-accent rounded-md shadow-sm"
                  >
                ...
                <button
                  onClick={() => handleBankAssignAll(stat.bank)}
                  disabled={!bankOfficerMap[stat.bank]}
                  className="mt-3 w-full inline-flex justify-center py-1.5 px-3 border border-transparent shadow-sm text-xs font-medium rounded-md text-white bg-primary hover:bg-primary-dark disabled:bg-gray-400 disabled:cursor-not-allowed"
                >
                  Assign All ({stat.caseCount})
                </button>
              </div>
            ))}
          </div>
